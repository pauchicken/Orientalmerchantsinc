...
TOKEN = os.getenv('TELEGRAM_BOT_TOKEN', '7879235294:AAGlc8j2ywcIhWhHTH6nEBtELST0CiNklYI')
...
# ------------------------------------------------------------------------------
# Utility function to find a product by item_code across nested dict
# ------------------------------------------------------------------------------
def find_product_by_item_code(item_code):
    """
    Given an item_code, search across the PRODUCTS dictionary
    (which is nested by brand -> category -> product list).
    Return the matching product dict if found, else None.
    """
    for brand, categories in PRODUCTS.items():
        for category_name, products_list in categories.items():
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [
            InlineKeyboardButton("✨ Shop", callback_data="view_products"),
            InlineKeyboardButton("👀 Profile", callback_data="view_profile"),
        ],
        [
            InlineKeyboardButton("🛒 Cart", callback_data="view_cart"),
            InlineKeyboardButton("📦 Orders", callback_data="view_orders"),
        ],
        [
            InlineKeyboardButton("🔍 Search", callback_data="search_products"),
            InlineKeyboardButton("❤️ Favorites", callback_data="view_favorites"),
        ],
    ]
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        [
            InlineKeyboardButton("✨ Add", callback_data="view_products"),
            InlineKeyboardButton("👀 Manage", callback_data="view_profile"),
        ],
        [
            InlineKeyboardButton("🛒 Cart", callback_data="view_cart"),
            InlineKeyboardButton("📦 Orders", callback_data="view_orders"),
...
async def view_products_by_category(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Build the keyboard to display products (by their item_code)
    buttons = [
        InlineKeyboardButton(
            f"{prod['name']}",
            callback_data=f"product_{prod['item_code']}"
        )
...
async def view_categories_by_brand(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    buttons = [
        InlineKeyboardButton(
            f"{category}",
            callback_data=f"category_{brand}_{category}"
        )
...
async def view_favorites(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = []
    for item_code in user_favorites:
        product = find_product_by_item_code(item_code)
        if product:
            keyboard.append([
                InlineKeyboardButton(
                    f"❤️ {product['name']} - ₱{product['unit_price']}",
                    callback_data=f"product_{item_code}"
                )
            ])
...
async def product_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, product_id=None) -> None:
    ...
    # Extract item_code from callback if not passed directly
    if not product_id:
        product_id = query.data[len('product_'):]

    product = find_product_by_item_code(product_id)
    if not product:
    ...
    # Display product details (using packing instead of description)
...
async def toggle_favorite(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    product = find_product_by_item_code(product_id)
    if not product:
        await query.edit_message_text("Product does not exist.")
        return
    ...
    if product_id in FAVORITES[user_id]:
        ...
        await query.edit_message_text(
            text=f"❌ Removed '{product['name']}' from your favorites."
        )
    else:
        FAVORITES[user_id].append(product_id)
        save_favorites(FAVORITES)
        await query.edit_message_text(
            text=f"✅ Added '{product['name']}' to your favorites."
        )
...
async def view_cart(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for idx, product in enumerate(cart):
        text += f"{idx + 1}. {product['name']} - ₱{product['unit_price']}\n"
        total_price += float(product['unit_price'])
        keyboard.append([
            InlineKeyboardButton(
...
async def checkout_cart(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    total_price = sum(float(prod['unit_price']) for prod in cart)
    product_names = ', '.join(prod['name'] for prod in cart)

    await query.edit_message_text(
        text=(
            f"You're about to purchase:\n{product_names}\n"
...
async def buy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        text=(
            f"You've selected {product['name']} for ₱{product['unit_price']}.\n"
            f"Would you like delivery or pickup?"
        ),
        reply_markup=reply_markup
    )
...
async def process_order(update: Update, context: ContextTypes.DEFAULT_TYPE, products, method, is_bulk, address=None):
    ...
    total_price = sum(float(prod['unit_price']) for prod in products)
    product_names = ', '.join(prod['name'] for prod in products)
    ...
    if method == 'delivery' and address:
...
async def send_order_to_group(context: ContextTypes.DEFAULT_TYPE, order: dict) -> None:
    ...
    product_details = '\n'.join(f"- {prod['name']}: ₱{prod['unit_price']}" for prod in products)
    total_price = sum(float(prod['unit_price']) for prod in products)
...
async def view_orders(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in ORDERS or not ORDERS[user_id]:
        ...
        for i, order in enumerate(ORDERS[user_id], 1):
            product_names = ', '.join(prod['name'] for prod in order['products'])
            total_price = sum(float(prod['unit_price']) for prod in order['products'])
...
async def search_products_result(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # We have nested structure in PRODUCTS. We'll flatten out a search.
    matched = []
    for brand, categories in PRODUCTS.items():
        for category_name, products_list in categories.items():
            for prod in products_list:
                name_match = bool(pattern.search(prod['name']))
                packing_match = bool(pattern.search(prod.get('packing', '')))
                if name_match or packing_match:
                    matched.append(prod)
...
# ------------------------------------------------------------------------------
# 5. Image Handling (Payment Confirmation)
# ------------------------------------------------------------------------------
